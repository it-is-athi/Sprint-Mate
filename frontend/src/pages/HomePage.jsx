      ...
      <div className="p-4 flex items-center justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h4 className="text-white font-medium truncate">{task.name || task.task_title || task.title}</h4>
            ...
            {completedTodaysTasks.length === 0 ? (
              ...
              completedTodaysTasks.map(task => (
                <TaskCard 
                  ...
                  onNavigate={() => task.schedule_id && navigate(`/dashboard/schedules/tasks/${task.schedule_id._id}`)}
                  onStatusChange={(e) => {
                    e.stopPropagation(); // Prevent navigation - completed tasks cannot be reverted
                  }}
                />
              ))
            )}
